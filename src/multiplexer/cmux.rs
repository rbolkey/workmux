...
impl CmuxBackend {
    ...
    /// Find a workspace by title using `cmux find-window` for faster lookup.
    ///
    /// `find-window` does substring matching, so we filter results for exact
    /// title equality. This is faster than `list-workspaces` + linear scan
    /// because `find-window` narrows candidates server-side.
    fn find_workspace_by_title(title: &str) -> Result<Option<WorkspaceEntry>> {
        // Try find-window first (faster for single-workspace lookups)
        let result = Cmd::new("cmux")
            .args(&["find-window", title])
            .run_and_capture_stdout();

        match result {
            Ok(output) => {
                let trimmed = output.trim();
                // "No matches" or empty means not found
                if trimmed.is_empty() || trimmed == "No matches" {
                    return Ok(None);
                }

                // Parse text format: 'workspace:N  "title"' per line.
                // Filter for exact title match (find-window does substring matching).
                for line in trimmed.lines() {
                    let parts: Vec<&str> = line.splitn(2, char::is_whitespace).collect();
                    if parts.len() < 2 {
                        continue;
                    }
                    let ws_ref = parts[0].trim();
                    // Title is in quotes after whitespace: '  "title"'
                    let title_part = parts[1].trim().trim_matches('"');
                    if title_part == title {
                        // Found exact match — construct a WorkspaceEntry.
                        // We don't have selected/pinned/index from find-window,
                        // but callers only need ws_ref and title.
            ...
            Err(_) => {
                // find-window failed — fall back to list-workspaces + linear scan
    ...
    ///
    /// sidebar-state returns a non-JSON text format: one `key=value` pair per line.
    /// `--json` flag is silently ignored (still returns text). `split_once('=')` is
    /// used so values containing `=` (e.g., `env=FOO=BAR`) are preserved intact.
    /// Known keys include: `tab`, `color`, `cwd`, `focused_cwd`, `git_branch`,
    /// `pr`, `ports`, `progress`, `status_count`, `meta_block_count`, `log_count`.
    ///
    /// JSON output is blocked on upstream `cmux identify` being per-calling-process
    /// only — it cannot resolve CWD for arbitrary workspaces. All callers have
...
impl Multiplexer for CmuxBackend {
    ...
    fn split_pane(
        ...
        // (inherent cmux server-side latency, not a bug). Retry with
        // exponential backoff capped at 400ms, max ~2s total wait.
        // Measured p95 latency is under 500ms; `new-split --command` is not
        // supported, so respawn-pane retry remains necessary.
        if let Some(cmd_str) = command {
            let mut last_err = None;
            for attempt in 0..6u32 {
                let backoff_ms = std::cmp::min(100 * (1u64 << attempt), 400);
    ...
    fn respawn_pane(&self, pane_id: &str, cwd: &Path, cmd: Option<&str>) -> Result<String> {
        ...
        if let Some(command) = cmd {
            ...
            let full_command = format!("cd {} && {}", shell_quote(&cwd.to_string_lossy()), command);
            Cmd::new("cmux")
                .args(&[
                    "respawn-pane",
                    "--workspace",
