...
/// tmux and unix-pipe use this directly; cmux uses CMUX_HANDSHAKE_TIMEOUT_SECS
/// because workspace creation + shell init is slower.
const HANDSHAKE_TIMEOUT_SECS: u64 = 5;

/// Extended timeout for cmux pane handshake.  cmux workspace creation has
/// higher latency than tmux (surface allocation, shell startup on a
/// freshly-created workspace), especially under load when many workspaces
/// already exist.
const CMUX_HANDSHAKE_TIMEOUT_SECS: u64 = 10;
...
impl PaneHandshake for CmuxHandshake {
    ...
    fn wait(self: Box<Self>) -> Result<()> {
        ...
        let timeout_secs = CMUX_HANDSHAKE_TIMEOUT_SECS;
